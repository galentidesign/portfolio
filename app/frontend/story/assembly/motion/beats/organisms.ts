/**
 * Beat 3 — organisms (0.61–0.74). A Nav bar facsimile drops in with real
...
export function addToTimeline(tl: Timeline, ctx: BeatContext): void {
  const { ease, lead } = ctx
  const { start } = ctx.span('organisms')
  ...
  if (table) tl.to(table, { y: 0, duration: 11, ease: ease.move }, start + 1)
  // ...while its rows spring in, one after another.
  if (rows.length)
    tl.to(rows, { opacity: 1, y: 0, duration: 6, stagger: 1.8, ease: ease.spring }, start + 3)

  if (step)
    tl.to(step, { opacity: 0, y: -56, duration: 2, ease: ease.exit }, ctx.exitAt('organisms'))
}
